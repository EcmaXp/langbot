...
kubectl apply --context sigsrv-prod -n langbot -f langbot-policy.yaml
...
kubectl create secret generic --context sigsrv-prod -n langbot langbot-secret-gpt-4o \
  --from-literal=LANGBOT_DISCORD_TOKEN=(op read "$OP_LANGBOT_SECRET_PATH/gpt-4o/DISCORD_TOKEN") \
...
kubectl create secret generic --context sigsrv-prod -n langbot langbot-secret-claude \
  --from-literal=LANGBOT_DISCORD_TOKEN=(op read "$OP_LANGBOT_SECRET_PATH/claude/DISCORD_TOKEN") \
...
kubectl create secret generic --context sigsrv-prod -n langbot langbot-secret-gemini \
  --from-literal=LANGBOT_DISCORD_TOKEN=(op read "$OP_LANGBOT_SECRET_PATH/gemini/DISCORD_TOKEN") \
